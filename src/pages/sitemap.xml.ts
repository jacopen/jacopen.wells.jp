...
export const GET: APIRoute = async () => {
  ...
  // 内部ブログ記事のみを取得（Markdownファイルベース）
  const internalBlogPosts = await getCollection('blog');
  ...
  const blogUrls = internalBlogPosts.map(post => ({
    url: `${siteUrl}/blog/${post.slug}`,
    lastmod: post.data.pubDate ? new Date(post.data.pubDate).toISOString() : currentDate,
    changefreq: 'monthly',
    priority: '0.7'
  }));
  ...
  const presentationPaginationUrls = Array.from({ length: totalPresentationPages }, (_, i) => ({
